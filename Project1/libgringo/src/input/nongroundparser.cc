...
namespace Gringo {
	namespace Input {
		...
		/*
			This is where files are pushed.
		*/
		void NonGroundParser::pushFile(std::string &&file) {
			// filenames_ is of type std::set<std::string>
			// Sets are containsers that store unique elements following a specific order.
...
function main(prg)
	...
	while true do
		...
		bool NonGroundParser::parse() {
			// so it turns out there's something called "nongroundgrammar.yy". That is a file for a completely
			// different piece of software called "lex". It is for the parsing, and contains the dictionary of all the symbols.
			// I'm not quite sure where nongroundgrammar below comes from, i'm unable to locate the header file specified
			// even though the program compiles... ? But this is definitely how each file is parsed. I'm not quite
			// sure how it all works though.

			// I still need to see if i can capture the output of the parsed file, or at least see where it's sent/what's done
			// with it. Obviously the code below is all it takes to parse all the files.

			_startSymbol = NonGroundGrammar::parser::token::PARSE_LP;		// what is start symbol?
			NonGroundGrammar::parser parser(this);							// where is the nongroundgrammar object?
			_init();
			auto ret = parser.parse();
			filenames_.clear();
			return ret == 0;
