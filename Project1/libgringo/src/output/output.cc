	...
	namespace Output {
		...
		namespace {
			...
			struct LparseHandler : StmHandler {
				...
				virtual void operator()(PredicateDomain::element_type &head) {
					if (!head.second.hasUid()) { head.second.uid(out.newUid()); }
					out.printExternal(head.second.uid());
				}
		...
		void PlainLparseOutputter::printExternal(unsigned) { }
		...
		void OutputBase::external(PredicateDomain::element_type &head) {
			(*handler)(head);
		}
