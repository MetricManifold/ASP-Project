...
namespace Gringo {
	namespace Output {
		...
		ULit Conjunction::toLparse(LparseTranslator &x) {
			...
			SAuxAtom bdLit(std::make_shared<AuxAtom>(x.auxAtom()));
			ULitVec bd;
			for (ConjunctionState::Elem &y : repr->second.elems) {
				if (!y.second.empty()) {
					...
					if (incomplete && y.first && y.first->second.defined()) {
						...
						// y.second :- chk.
						for (auto &z : y.second) { z->makeEqual(make_unique<AuxLiteral>(chk, false), x); }
					}
					// body += aux
			...
			// bdLit :- body.
			LparseRule(bdLit, std::move(bd)).toLparse(x);
