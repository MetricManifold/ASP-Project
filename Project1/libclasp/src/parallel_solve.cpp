	...
	namespace mt {
		...
		struct ParallelSolve::SharedData {
			...
			enum MsgFlag {
				...
				allow_split_flag = 128u, // set if splitting mode is active
...
#pragma message TODO("replace with CACHE_LINE_ALIGNED alloc")
		...
		void ParallelSolve::solveParallel(uint32 id) {
			...
			try {
				...
				for (ValueRep last = value_free; requestWork(s, a); solve.reset()) {
					agg.accu(s.stats);
					s.stats.reset();
					last = thread_[id]->solveGP(solve, *a, a.is_owner() ? gp_split : initialGp_, shared_->maxConflict);
		...
		void ParallelSolve::terminate(Solver& s, bool complete) {
			if (!shared_->terminate()) {
				if (complete && enumerator().optimize() > 1 && s.popRootLevel(s.rootLevel())) {
					...
					assert(s.decisionLevel() == 0);
					complete = false;
					reportProgress(MessageEvent(s, "SYNC-UNSAT", MessageEvent::sent));
		...
		bool ParallelSolve::waitOnSync(Solver& s) {
			...
			if (shared_->workSem.wait()) {
				...
				else if (unsat && !enumerator().commitUnsat(s, true)) {
					// nothing more to do
					init = false;
				...
				shared_->syncT.lap();
				reportProgress(MessageEvent(s, unsat ? "SYNC-UNSAT" : "SYNC", MessageEvent::completed, shared_->syncT.elapsed()));
		...
		void ParallelSolve::initQueue() {
			shared_->clearQueue();
			int end = shared_->hasControl(SharedData::allow_split_flag) ? 1 : numThreads();
		...
		bool ParallelSolve::commitModel(Solver& s) {
			bool interrupt = false;
			{
				...
				bool isModel = thread_[s.id()]->isModel(s);
				interrupt = shared_->terminate();
				while (!interrupt && (isModel || enumerator().hasModel(s))) {
					Model m = enumerator().commitModel(s);
					if (m.num == 1 && !enumerator().supportsRestarts()) {
					...
					if (isModel) { ++shared_->modCount; isModel = false; }
					if (!reportModel(s, m)) {
					...
					interrupt = shared_->interrupt();
				}
			} // end of critical section
			return !interrupt && (!s.hasConflict() || s.resolveConflict());
		}
		...
		bool ParallelSolve::integrateModels(Solver& s, uint32& upCount) {
			...
			return (gCount == upCount || enumerator().hasModel(s))
				|| (enumerator().update(s) && (upCount = gCount) == gCount);
		}
		...
		ValueRep ParallelHandler::solveGP(BasicSolve& solve, const LitVec& gp, GpType t, uint64 restart) {
			gp_.reset(restart, t);
			up_ = 1;
			act_ = 0;
			const_cast<const ParallelSolve*>(ctrl_)->enumerator()->setDisjoint(solve.solver(), disjointPath());
			if (!solve.assume(gp)) { return value_false; }
			act_ = 1; // activate enumerator and bounds
			ValueRep res = value_free;
			for (Solver& s = solve.solver();;) {
				ctrl_->integrateModels(s, gp_.modCount);
				if ((res = solve.solve()) != value_true) { break; }
				if (!ctrl_->commitModel(s)) { res = value_false; break; }
			}
			act_ = 0;
			return res;
		}
		...
		bool ParallelHandler::propagateFixpoint(Solver& s, PostPropagator* ctx) {
			...
			if (int up = (ctx == 0 && s.isTrue(s.sharedContext()->tagLiteral()) && up_ != 0)) {
