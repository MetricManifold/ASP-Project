...
namespace Clasp {
	...
	struct ClaspFacade::SolveImpl {
		...
		void solve(ClaspFacade& f, EventHandler* h, bool async) {
			check_precondition(f.config_);
			check_precondition(ready() && "Concurrent solve operations not supported!");
	...
	SatBuilder& ClaspFacade::startSat(ClaspConfig& config, bool allowUpdate) {
		config.setDefaults(Problem_t::SAT);
		init(config, true);
		initBuilder(new SatBuilder(config.enumerate.opt.maxSat), allowUpdate);
		return static_cast<SatBuilder&>(*builder_.get());
	}

	PBBuilder& ClaspFacade::startPB(ClaspConfig& config, bool allowUpdate) {
		config.setDefaults(Problem_t::PB);
		init(config, true);
	...
	Asp::LogicProgram& ClaspFacade::startAsp(ClaspConfig& config, bool allowUpdate) {
		config.setDefaults(Problem_t::ASP);
		init(config, true);
	...
	ProgramBuilder& ClaspFacade::update(bool reloadConfig) {
		check_precondition(builder_.get());
		check_precondition(solve_->ready());
		check_precondition(step_.result.signal != SIGINT && "Interrupt not handled!");
		solve_->reset();
	...
	bool ClaspFacade::prepare(EnumMode enumMode) {
		...
		if (builder_.get() && (ok = builder_->endProgram()) == true) {
			...
			if ((m = !en.opt.disabled() ? builder_->getMinimizeConstraint(&en.opt.vals) : 0) != 0 && !m->setMode(static_cast<MinimizeMode_t::Mode>(en.opt.mode), en.opt.vals)) {
				m = 0;
				ok = false;
			}
		}
		if (ok && (!ctx.frozen() || (ok = ctx.unfreeze()) == true)) {
			if ((!m || (m->numRules() == 1 && en.opt.mode < MinimizeMode_t::opt_inc)) && !isSentinel(ctx.tagLiteral())) {
				// Tag literal not needed in this step.
				ok = ctx.addUnary(ctx.tagLiteral());
			}
			if (ok && enumMode == enum_static) {
	...
	const ClaspFacade::Summary& ClaspFacade::shutdown() {
		if (solving()) { terminate(SIGINT); solve_->join(); }
...
#define COMMON_KEYS "ctx.\0solvers.\0solver.\0totalTime\0cpuTime\0solveTime\0unsatTime\0satTime\0numEnum\0step\0result\0"
		static const char* _keys[] = { "accu.\0hccs.\0hcc.\0lp.\0" COMMON_KEYS, 0, "accu.\0lp.\0" COMMON_KEYS, "accu.\0" COMMON_KEYS };
		enum ObjId { id_hccs, id_hcc, id_lp, id_ctx, id_solvers, id_solver, id_total, id_cpu, id_solve, id_unsat, id_sat, id_num, id_step, id_result };
		if (!path)  path = "";
		std::size_t kLen = 0;
		...
		for (const char* k = keyL + 6; (kLen = std::strlen(k)) != 0; k += kLen + 1, ++oId) {
			...
			if (match) {
				...
				case id_hccs: if (!ctx.sccGraph.get() || ctx.sccGraph->numNonHcfs() == 0) { return ExpectedQuantity::error_not_available; }
							  else {
...
#undef COMMON_KEYS
	...
	int ClaspFacade::Summary::optimize()       const { return facade->enum_.get() ? facade->enum_->optimize() : 0; }
	uint64 ClaspFacade::Summary::models()      const { return facade->enum_.get() && facade->enum_->exhaustive() ? std::min(enumerated(), uint64(1)) : enumerated(); }
