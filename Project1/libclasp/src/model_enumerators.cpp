...
#include "../../libclasp/clasp/util/multi_queue.h"
#include <algorithm>
#include <cstdlib>
namespace Clasp {
	...
	public:
		...
		bool doUpdate(Solver& s, bool disjoint);
		bool simplify(Solver& s, bool) { simplifyDB(s, db, false); return false; }
		QPtr          queue;
		ThreadId      id;
		ConstraintDB  db;
		LitVec        solution;
	...
	bool ModelEnumerator::RecordFinder::doUpdate(Solver& s, bool) {
		if (queue) {
	...
	void ModelEnumerator::RecordFinder::doCommitModel(Enumerator& x, Solver& s) {
		...
		if (!ctx.projectionEnabled()) {
			for (uint32 x = s.decisionLevel(); x != 0; --x) {
				solution.push_back(~s.decision(x));
			}
		}
		else {
	...
	void ModelEnumerator::BacktrackFinder::undoLevel(Solver& s) {
	...
	bool ModelEnumerator::BacktrackFinder::doUpdate(Solver& s, bool disjoint) {
		if (hasModel()) {
			bool ok = true;
			...
			if (front == 0) { // The decision stack is already ordered.
				...
				c = Clause::newContractedClause(s, rep, back, false);
				s.assume(~x);
				...
				// Once the solver removes that level, the nogood (which is then satisfied) is destroyed.
				// Hence, the number of projection nogoods is linear in the number of (projection) atoms.
	...
	void ModelEnumerator::BacktrackFinder::doCommitModel(Enumerator& ctx, Solver& s) {
		...
		if (!en.projectionEnabled()) {
			...
			for (uint32 r = btLev + 1, end = s.decisionLevel(); r <= end && s.sharedContext()->project(s.decision(r).var()); ++r) {
				++btLev;
			}
	...
	EnumerationConstraint* ModelEnumerator::doInit(SharedContext& ctx, MinimizeConstraint* min, int numModels) {
		...
		if (projectionEnabled()) { c->setIgnoreSymmetric(true); }
		return c;
	}
