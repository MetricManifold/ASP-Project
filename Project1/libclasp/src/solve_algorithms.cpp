...
namespace Clasp {
	...
	BasicSolve::BasicSolve(Solver& s, SolveLimits* lim) : solver_(&s), params_(&s.sharedContext()->configuration()->search(s.id())), limits_(lim), state_(0) {}
	...
	bool SolveAlgorithm::reportModel(const Solver& s, const Model& m) const {
		bool r1 = !onModel_ || onModel_->onModel(s, m);
		bool r2 = s.sharedContext()->report(s, m);
		return r1 && r2 && enumLimit_ > m.num;
	}
	/////////////////////////////////////////////////////////////////////////////////////////
	// SequentialSolve
	/////////////////////////////////////////////////////////////////////////////////////////
	...
	bool SequentialSolve::doSolve(SharedContext& ctx, const LitVec& path) {
		Solver&        s = *ctx.master();
		SolveLimits  lim = limits();
		uint32      root = s.rootLevel();
		ValueRep     res = value_free;
		ctx.attach(s);
		if (term_) { term_->attach(s); }
		for (BasicSolve solve(s, ctx.configuration()->search(0), &lim); res == value_free && !interrupted(); solve.reset()) {
			// Add assumptions - if this fails, the problem is unsat 
			// under the current assumptions but not necessarily unsat.
			res = value_false;
			if (solve.assume(path) && enumerator().update(s)) {
				while ((enumerator().hasModel(s) || (res = solve.solve()) == value_true) && reportModel(s, enumerator().commitModel(s))) {
					enumerator().update(s);
				}
			}
			if (res != value_false || interrupted()) { res = value_free; break; }
			if (enumerator().commitUnsat(s, true)) { res = value_free; }
		}
		s.popRootLevel(s.rootLevel() - root);
		setLimits(lim);
		if (term_) { term_->detach(); }
		ctx.detach(s);
		return res == value_free;
	}
