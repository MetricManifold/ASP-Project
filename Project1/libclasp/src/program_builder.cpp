...
namespace Clasp {
	...
	bool ProgramBuilder::updateProgram() {
		check_precondition(ctx_ && "startProgram() not called");
	...
	void ProgramBuilder::getAssumptions(LitVec& out) const {
		check_precondition(ctx_ && frozen());
	...
	void SatBuilder::prepareProblem(uint32 numVars, wsum_t cw, uint32 clauseHint) {
		check_precondition(ctx() && "startProgram() not called!");
		ctx()->reserveVars(numVars + 1);
		for (Var v = ctx()->numVars() + 1; v <= numVars; ++v) { ctx()->addVar(Var_t::atom_var); }
	...
	bool SatBuilder::addClause(LitVec& clause, wsum_t cw) {
		...
		check_precondition(cw >= 0 && cw <= std::numeric_limits<weight_t>::max());
	...
	bool SatBuilder::doEndProgram() {
		...
		if (!softClauses_.empty() && ok) {
			...
			ctx()->reserveVars(vars_ + 1);
			for (Var v = ctx()->numVars(); v != vars_;) { v = ctx()->addVar(Var_t::atom_var); }
	...
	void PBBuilder::prepareProblem(uint32 numVars, uint32 numProd, uint32 numSoft, uint32 numCons) {
		...
		ctx()->symTab().startInit();
		for (Var v = ctx()->numVars() + 1; v <= maxVar; ++v) {
			maxVar_ = ctx()->addVar(Var_t::atom_var);
		}
		ctx()->symTab().endInit(SymbolTable::map_direct, numVars + 1);
		ctx()->startAddConstraints(numCons);
	}
	uint32 PBBuilder::getNextVar() {
		check_precondition(ctx()->validVar(++nextVar_));
		return nextVar_;
	}
	bool PBBuilder::addConstraint(WeightLitVec& lits, weight_t bound, bool eq, weight_t cw) {
		...
		return !eq ? WeightConstraint::newWeightConstraint(*ctx(), posLit(eqVar), lits, bound) : WeightConstraint::newWeightEqConstraint(*ctx(), posLit(eqVar), lits, bound);
	}
	...
	void PBBuilder::addProductConstraints(Literal eqLit, LitVec& lits) {
		...
		for (LitVec::iterator it = lits.begin(), end = lits.end(); it != end && ok; ++it) {
			...
			ok = ClauseCreator::create(s, ShortRep(~eqLit, *it), 0).ok();
			*it = ~*it;
		}
