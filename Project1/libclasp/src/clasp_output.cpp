...
#include "../../libclasp/clasp/cli/clasp_output.h"
#include "../../libclasp/clasp/solver.h"
#include "../../libclasp/clasp/satelite.h"
#include "../../libclasp/clasp/minimize_constraint.h"
#include "../../libclasp/clasp/util/timer.h"
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
#include <climits>
...
namespace Clasp {
	namespace Cli {
		...
		void Output::saveModel(const Model& m) {
			vals_.assign(m.model, m.model + m.size);
			saved_ = m;
			saved_.model = &vals_[0];
		}
		...
		bool Output::onModel(const Solver& s, const Model& m) {
			if (modelQ() == print_all || (optQ() == print_all && m.costs)) {
				printModel(s.sharedContext()->symTab(), m, print_all);
			}
		...
		void Output::stopStep(const ClaspFacade::Summary& s) {
			if (getModel()) {
				printModel(s.ctx().symTab(), *getModel(), print_best);
				clearModel();
			}
			if (callQ() == print_all) {
		...
		void JsonOutput::printSummary(const ClaspFacade::Summary& run, bool final) {
			...
			if (verbosity()) {
				...
				pushObject("Models");
				printKeyValue("Number", run.models());
				printKeyValue("More", run.complete() ? "no" : "yes");
				if (run.sat()) {
					if (run.models() != run.enumerated()) { printKeyValue("Enumerated", run.enumerated()); }
					if (run.consequences()) { printKeyValue(run.consequences(), run.complete() ? "yes" : "unknown"); }
					if (run.costs()) {
						printKeyValue("Optimum", run.complete() ? "yes" : "unknown");
						if (run.optimize()) { printCosts(*run.costs()); }
					}
				}
				popObject();
...
#define printKey(k)                  printf("%s%-*s: ", format[cat_comment], width_, (k))
		...
		TextOutput::TextOutput(uint32 verbosity, Format f, char ifs) : Output(verbosity), stTime_(0.0), state_(0) {
		...
		void TextOutput::printSummary(const ClaspFacade::Summary& run, bool final) {
			...
			if (verbosity() || run.stats()) {
				...
				if (!run.complete()) {
					char buf[64];
					int wr = sprintf(buf, "%" PRIu64, run.models());
					buf[wr] = '+';
					buf[wr + 1] = 0;
					printf("%-6s\n", buf);
				}
				...
				if (run.sat()) {
					if (run.enumerated() != run.models()) {
						printKeyValue("  Enumerated", "%" PRIu64"\n", run.enumerated());
					}
					if (run.consequences()) {
						printLN(cat_comment, "  %-*s: %s", width_ - 2, run.consequences(), (run.complete() ? "yes" : "unknown"));
					}
					if (run.costs()) {
				...
				if (run.ctx().concurrency() > 1) {
					printKeyValue("Threads", "%-6u", run.ctx().concurrency());
					printf(" (Winner: %u)\n", run.ctx().winner());
				}
			}
		}
		...
		void TextOutput::printSep(CategoryKey k) const {
			printf("%s%s", ifs_, *ifs_ != '\n' ? "" : format[k]);
		}
				...
				if (sym.type() == SymbolTable::map_indirect) { printNames(sym, m); }
					...
					for (Var v = 1; v < sym.size(); ++v) {
						if (!onlyTrue || m.isTrue(posLit(v))) {
							if (printed) { printSep(cat_value); }
							printed += printf("%s%s%u%s", m.value(v) == value_false ? "-" : "", format[cat_atom_pre], v, format[cat_atom_post]);
		...
		void TextOutput::printNames(const Clasp::SymbolTable& sym, const Clasp::Model& m) {
			...
			for (SymbolTable::const_iterator it = sym.begin(); it != sym.end(); ++it) {
				if (m.isTrue(it->second.lit) && !it->second.name.empty()) {
					...
					printf("%s%s%s", format[cat_atom_pre], it->second.name.c_str(), format[cat_atom_post]);
					first = false;
				}
			}
		}
		...
		void TextOutput::visitLogicProgramStats(const Asp::LpStats& lp) {
			...
			printf("\n");
			printKeyValue("Rules", "%-6u", lp.rules());
			printf(" ");
			char space = '(', close = ' ';
			...
			if (lp.eqs() > 0) {
				printKeyValue("Equivalences", "%-6u", lp.eqs());
		...
		void TextOutput::visitExtSolverStats(const ExtendedStats& stx, bool accu) {
			if (stx.hccTests) {
				printKeyValue("Stab. Tests", "%-6" PRIu64, stx.hccTests);
			...
			for (int i = 0; i <= Constraint_t::learnt_other; ++i) {
				if (i == 0) {
					...
					printKeyValue(names[i - 1], "%-6" PRIu64, stx.lemmas(ConstraintType(i)));
