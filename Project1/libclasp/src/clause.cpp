...
//
#include "../../libclasp/clasp/clause.h"
#include "../../libclasp/clasp/solver.h"
#include "../../libclasp/clasp/util/misc_types.h"
#include <algorithm>
...
#pragma message TODO("replace with CACHE_LINE_ALIGNED alloc")
	...
	ClauseRep ClauseCreator::prepare(Solver& s, LitVec& lits, uint32 flags, const ClauseInfo& info) {
		...
		ClauseRep x = { &lits[0], (uint32)lits.size(), info };
		return x;
	}
	...
	bool ClauseCreator::ignoreClause(const Solver& s, const ClauseRep& c, Status st, uint32 flags) {
		if (st == status_sat) {
			return (flags & clause_not_sat) != 0 || ((flags & clause_not_root_sat) != 0 && s.level(c.lits[0].var()) <= s.rootLevel());
		}
		return st == status_subsumed || ((st & status_unsat) != 0 && (flags & clause_not_conflict) != 0);
	}
	...
	ClauseHead* ClauseCreator::newLearntClause(Solver& s, const ClauseRep& clause, uint32 flags) {
		...
		sharedPtr.clause = s.sharedContext()->distribute(s, clause.lits, clause.size, clause.info);
		if (clause.size <= Clause::MAX_SHORT_LEN || sharedPtr.clause == 0) {
			if (!s.isFalse(clause.lits[1]) || !s.strategy.compress || clause.size < s.strategy.compress) {
				...
				std::stable_sort(clause.lits + 2, clause.lits + clause.size, Detail::GreaterLevel(s));
	...
	ClauseCreator::Result ClauseCreator::create_prepared(Solver& s, const ClauseRep& clause, uint32 flags) {
		...
		if (clause.size > 1) {
			...
			Result ret(0, x);
			if (clause.size > 3 || clause.info.tagged() || (flags&clause_explicit) != 0 || !s.addShort(clause)) {
		...
		Literal imp = clause.size == 1 ? clause.lits[0] : negLit(0);
		return Result(0, s.addUnary(imp, clause.info.type()) ? status_unit : status_unsat);
	}
	...
	ClauseCreator::Result ClauseCreator::create(Solver& s, const ClauseRep& rep, uint32 flags) {
		return create_prepared(s
			, (flags & clause_no_prepare) != 0 ? rep : prepare(s, rep.lits, rep.size, rep.info, flags, rep.lits)
			, flags);
	}
	...
	namespace mt {
		...
		bool SharedLitsClause::simplify(Solver& s, bool reinit) {
			...
			if (optSize == 0) {
				...
				ClauseRep raw = { lits, static_cast<uint32>(j - lits),  ClauseInfo() };
				ClauseHead* h = Clause::newClause(this, s, raw);
