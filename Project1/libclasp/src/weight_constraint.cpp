...
namespace Clasp {
	...
	WeightLitsRep WeightLitsRep::create(Solver& s, WeightLitVec& lits, weight_t bound) {
		...
		weight_t sumW = 0;
		uint32 minW = std::numeric_limits<uint32>::max(), maxW = 1;
		weight_t B = std::max(bound, 1);
		for (LitVec::size_type i = 0; i != lits.size(); ++i) {
			assert(lits[i].second > 0);
			s.clearSeen(lits[i].first.var());
			if (uint32(lits[i].second) > maxW) { maxW = lits[i].second = std::min(lits[i].second, B); }
	...
	bool WeightLitsRep::propagate(Solver& s, Literal W) {
		if (bound <= 0) { return s.addUnary(W, Constraint_t::static_constraint); } // trivially SAT
		else if (bound > reach) { return s.addUnary(~W, Constraint_t::static_constraint); } // trivially UNSAT
		else if (s.value(W.var()) == value_free) { return true; }
		// backward propagate
	...
	// WeightConstraint
	/////////////////////////////////////////////////////////////////////////////////////////
	bool WeightConstraint::newWeightConstraint(SharedContext& ctx, Literal W, WeightLitVec& lits, weight_t bound, Constraint** out) {
		WeightLitsRep rep = WeightLitsRep::create(*ctx.master(), lits, bound);
		return newWeightConstraint(ctx, W, rep, out);
	}
	// Adds constraints for W == (lits == bound)
	bool WeightConstraint::newWeightEqConstraint(SharedContext& ctx, Literal W, WeightLitVec& lits, weight_t bound) {
		WeightLitsRep rep = WeightLitsRep::create(*ctx.master(), lits, bound);
		return newWeightConstraint(ctx, W, rep)
			&& (++rep.bound, newWeightConstraint(ctx, ~W, rep));
	}
	bool WeightConstraint::newWeightConstraint(SharedContext& ctx, Literal W, WeightLitsRep& rep, Constraint** out) {
		if (out) *out = 0;
		Solver& s = *ctx.master();
		if (!rep.propagate(s, W)) { return false; }
		if (!rep.open()) { return true; }
		if (rep.bound == 1 || rep.bound == rep.reach) {
			...
			for (LitVec::size_type i = 0; i != rep.size; ++i) {
				Literal x = ~clause[0];
				Literal y = rep.lits[i].first ^ disj;
				if (x != ~y) {
					if (x != y) { clause.push_back(~y); }
					if (!ClauseCreator::create(s, ShortRep(x, y), 0)) { return false; }
				}
				else { sat = true; }
			}
			return sat || ClauseCreator::create(s, clause, 0);
		}
		...
		Constraint* c = new (m) WeightConstraint(ctx, W, rep, sL);
		if (out) *out = c;
		s.add(c);
		return true;
	}

	WeightConstraint::WeightConstraint(SharedContext& ctx, Literal W, const WeightLitsRep& rep, WL* out) {
