...
#undef RETURN_IF
		...
		bool LogicProgram::doUpdateProgram() {
			check_precondition(frozen() || !incData_);
		...
		void LogicProgram::write(std::ostream& os) {
			...
			Literal comp;
			SymbolTable::const_iterator sym = ctx()->symTab().begin();
			for (AtomList::size_type i = 1; i < atoms_.size(); ++i) {
				...
				if (sym != ctx()->symTab().end() && Var(i) == sym->first) {
		...
		/////////////////////////////////////////////////////////////////////////////////////////
#define check_not_frozen() check_precondition(!frozen() && "Can't update frozen program!")
#define check_modular(x, atomId) (void)( (!!(x)) || (throw RedefinitionError((atomId), this->getAtomName((atomId))), 0))
		...
		LogicProgram& LogicProgram::setAtomName(Var atomId, const char* name) {
			...
			resize(atomId);
			ctx()->symTab().addUnique(atomId, name);
			return *this;
		}
		...
		LogicProgram& LogicProgram::freeze(Var atomId, ValueRep value) {
			check_not_frozen();
			check_precondition(incData_ && "LogicProgram::updateProgram() not called!");
			assert(incData_->frozen.empty());
			PrgAtom* a = resize(atomId);
			if (!a->inFlux() && (a->frozen() || (atomId >= startAtom() && !a->supports()))) {
				check_precondition(value == value_false || value == value_true);
...
#undef check_not_frozen
		...
		Literal LogicProgram::getLiteral(Var atomId) const {
			check_precondition(atomId < atoms_.size());
		...
		void LogicProgram::addRuleImpl(RuleType r, const VarVec& heads, BodyInfo& body) {
			if (r != OPTIMIZERULE) {
				...
				check_precondition(heads.empty());
		...
		bool LogicProgram::addConstraints() {
			...
			ctx()->symTab().endInit();
			check_precondition(ctx()->symTab().curBegin() == ctx()->symTab().end() || startAtom() <= ctx()->symTab().curBegin()->first);
			...
			symbol_iterator sym = ctx()->symTab().lower_bound(ctx()->symTab().curBegin(), startAtom());
			symbol_iterator symEnd = ctx()->symTab().end();
...
#undef check_modular
		...
		RuleType LogicProgram::simplifyBody(const Rule& r, BodyInfo& info) {
			...
			for (WeightLitVec::const_iterator it = r.body.begin(), bEnd = r.body.end(); it != bEnd; ++it) {
				...
				check_precondition(it->second > 0);
				...
				if (!ruleState_.inBody(lit)) {  // literal not seen yet
					...
					check_precondition((INT_MAX - oldW) >= w && "Integer overflow!");
		...
		const char* LogicProgram::getAtomName(Var id) const {
			if (const SymbolTable::symbol_type* x = ctx()->symTab().find(id)) {
				return x->name.c_str();
			}
			return "";
		}
