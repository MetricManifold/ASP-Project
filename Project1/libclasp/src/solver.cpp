...
namespace Clasp {
	...
	void Solver::startInit(uint32 numConsGuess) {
		...
		if (strategy.loadCfg == 1) {
			...
			if (!shared_->seedSolvers()) { rng.srand(params.seed); }
			else {
				RNG x(14182940); for (uint32 i = 0; i != id; ++i) { x.rand(); }
				rng.srand(x.seed());
			}
		}
		if (params.heuReinit || heuristic_.get() == 0) {
	...
	bool Solver::addShort(const ClauseRep& rep) {
		bool learnt = rep.info.learnt();
		if (!rep.isImp() || (learnt && !shared_->allowImplicit(rep.info.type()))) {
			return false;
		}
		const Literal* c = rep.lits;
		learnt = (rep.size == 3 ? shared_->addTernary(c[0], c[1], c[2], learnt) : shared_->addBinary(c[0], c[1], learnt)) && learnt;
		if (learnt) {
			stats.addLearnt(rep.size, rep.info.type());
			shared_->distribute(*this, rep.lits, rep.size, rep.info);
		}
		return true;
	}
	...
	bool Solver::pushRoot(const LitVec& path) {
		...
		undoUntil(0, true);
		if (hasConflict() || !simplify() || !propagate() || !pushRoot(shared_->stepLiteral())) { return false; }
		...
		return pushRoot(shared_->tagLiteral());
	}
	...
	bool Solver::popRootLevel(uint32 i) {
		...
		bool tagActive = isTrue(sharedContext()->tagLiteral());
		// go back to new root level and re-assert still implied literals
		undoUntil(rootLevel(), true);
		if (tagActive && !isTrue(sharedContext()->tagLiteral())) {
			removeConditional();
		}
		return !hasConflict();
	}
	...
	bool Solver::simplifySAT() {
		...
		lastSimp_ = (uint32)assign_.front;
		if (shufSimp_) {
		...
		if (this == sharedContext()->master()) { shared_->simplifyDB(shuffle); }
