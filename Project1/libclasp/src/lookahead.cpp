...
//
#include "../../libclasp/clasp/lookahead.h"
#include <algorithm>
namespace Clasp {
	/////////////////////////////////////////////////////////////////////////////////////////
	// Lookahead scoring
	...
	void UnitHeuristic::resurrect(const Solver& s, Var v) {
		if ((s.sharedContext()->type(v) & look_->score.types) != 0) {
			look_->score.score.resize(s.numVars() + 1);
			look_->append(s.sharedContext()->preferredLiteralByType(v),
				s.sharedContext()->type(v) == Var_t::atom_body_var || look_->score.types != Var_t::atom_body_var);
		}
	}
	/////////////////////////////////////////////////////////////////////////////////////////
