...
namespace Clasp {
	namespace Asp {
			...
			for (EdgeVec::iterator it = supports_.begin(); it != supports_.end(); ++it) {
				...
				if (n->relevant() && n->hasVar()) {
					*j++ = *it;
					...
					if (it->isNormal() && !ClauseCreator::create(*ctx.master(), ShortRep(literal(), ~B), 0)) { // FTA/BFA
						return false;
					}
				}
			}
		...
		bool PrgBody::mergeHeads(LogicProgram& prg, PrgBody& heads, bool strong) {
		...
		bool PrgBody::addConstraints(const LogicProgram& prg, ClauseCreator& gc) {
			...
			return WeightConstraint::newWeightConstraint(*prg.ctx(), literal(), lits, bound());
		}
