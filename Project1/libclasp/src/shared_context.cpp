...
#undef FOR_EACH_LEARNT
	...
	bool SatPreprocessor::preprocess(SharedContext& ctx, Options& opts) {
		...
		for (ClauseList::size_type i = 0; i != clauses_.size(); ++i) {
			if (clauses_[i]) {
				Clause& c = *clauses_[i];
				ClauseRep rep = { &c[0], c.size(), ClauseInfo() };
				if (!ClauseCreator::create(*s, rep, 0)) {
					return false;
				}
				c.destroy();
			}
	...
	void SharedContext::cloneVars(const SharedContext& other, InitMode m) {
		...
		if (&symTab() != &other.symTab()) {
			if (m == init_copy_symbols) { other.symTab().copyTo(symTab()); }
			else {
	...
	bool SharedContext::unfreezeStep(Solver& s) {
		assert(!s.decisionLevel());
		if (s.value(step_.var()) == value_free) { s.force(~step_, posLit(0)); }
		if (s.value(tag_.var()) == value_free) { s.force(~tag_, posLit(0)); }
		uint32 rem = !isSentinel(step_) + !isSentinel(tag_);
		if (rem && s.simplify()) {
			LitVec::size_type i = s.assign_.trail.size();
			while (i--) {
				Var v = s.assign_.trail[i].var();
				if (v == step_.var() || v == tag_.var()) {
	...
	void SharedContext::requestTagLiteral() { if (tag_ == posLit(0)) { tag_ = negLit(0); } }
	void SharedContext::requestStepLiteral() { if (step_ == posLit(0)) { step_ = negLit(0); } }
	...
	Solver& SharedContext::startAddConstraints(uint32 constraintGuess) {
		...
		if (master()->isFalse(step_)) {
			step_ = posLit(addVar(Var_t::atom_var));
			setFrozen(step_.var(), true);
			--problem_.vars;
		}
	...
	bool SharedContext::attach(Solver& other) {
		...
		if (other.validVar(tag_.var()) && other.validVar(step_.var())) {
			if (!other.popRootLevel(other.rootLevel())) { return false; }
			if (&other == master()) { return true; }
			if (!unfreezeStep(other)) { return false; }
		}
		initStats(other);
	...
	void SharedContext::simplifyDB(bool shuffle) {
	...
	SharedLiterals* SharedContext::distribute(const Solver& s, const Literal* lits, uint32 size, const ClauseInfo& extra) const {
		if (distributor.get() && !extra.tagged() && (size <= 3 || distributor->isCandidate(size, extra.lbd(), extra.type()))) {
