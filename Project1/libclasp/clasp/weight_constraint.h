...
namespace Clasp {
	...
	struct WeightLitsRep {
		...
		bool open()       const { return bound != 0 && reach >= bound; }
	...
	public:
		 ...
		 * the new weight constraint is added to s.
		...
		static bool newWeightConstraint(SharedContext& ctx, Literal W, WeightLitsRep& rep, Constraint** out = 0);
		//! Adds W == (lits == bound).
		static bool newWeightEqConstraint(SharedContext& ctx, Literal W, WeightLitVec& lits, weight_t bound);
	...
	private:
		...
		WeightConstraint(SharedContext& ctx, Literal W, const WeightLitsRep&, WL* out);
