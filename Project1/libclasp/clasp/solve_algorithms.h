...
namespace Clasp {
	...
	protected:
		...
		bool          reportModel(const Solver& s, const Model& m) const;
...
#define CLASP_BASIC_SOLVE_OPTIONS(OPTION, SELF) \
	OPTION("solve-limit", solve_limit, X(arg("<n>[,<m>]")), "Stop search after <n> conflicts or <m> restarts\n", {\
	UPair arg(UINT32_MAX, UINT32_MAX);\
	return ((isDisabled(value) && *value != '0') || store(value,arg)) && (SELF.limit=SolveLimits(arg.first == UINT32_MAX ? UINT64_MAX : arg.first, arg.second == UINT32_MAX ? UINT64_MAX : arg.second), true);})
