...
namespace Clasp {
	namespace Cli {
		...
		protected:
			const Model* getModel() const { return saved_.model ? &saved_ : 0; }
			void         saveModel(const Model& m);
			void         clearModel() { saved_.model = 0; }
		private:
			Output(const Output&);
		...
		public:
			...
			enum CategoryKey { cat_comment, cat_value, cat_objective, cat_result, cat_value_term, cat_atom_pre, cat_atom_post, num_cat };

			const char* result[num_str]; /**< Default result strings. */
			const char* format[num_cat]; /**< Format strings.         */

			TextOutput(uint32 verbosity, Format f, char ifs = ' ');
			~TextOutput();
			 ...
			 * elements e are enclosed in the (possibly empty) strings format[cat_atom_pre]/format[cat_atom_post]
			 * and separated by the internal field separator.
			 */
		...
		private:
			...
			void printSep(CategoryKey c) const;
