...
namespace Clasp {
	...
	public:
		...
		//! Adds the short constraint con to the short implication graph if possible.
		/*!
		 * \note If con can't be stored in the short implication graph (because updates are
		 *       not permitted or con is not a short implication), the function
		 *       returns false without adding con. Otherwise, it also publishes con
		 *       via a call to SharedContext::distribute().
		 *
		 * \see  SharedContext::allowImplicit(ConstraintType)
		 */
		bool addShort(const ClauseRep& con);
	...
	public:
		...
		static Literal defaultLiteral(Solver& s, Var v) {
			...
			default: // 
			case SolverStrategies::sign_atom: return Literal(v, !s.sharedContext()->info(v).has(VarInfo::BODY));
			...
			case SolverStrategies::sign_disj: return Literal(v, !s.sharedContext()->info(v).has(VarInfo::BODY | VarInfo::DISJ));
			}
		}
	private:
