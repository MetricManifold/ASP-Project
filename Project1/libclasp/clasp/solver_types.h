...
#include "util/left_right_sequence.h"
#include "util/misc_types.h"
#include "util/type_manip.h"
#include <numeric>
/*!
 * \file
...
#define CLASP_CORE_STATS(STAT, SELF, OTHER)     \
	...
	STAT(uint64 lastRestart;, STRING(lastRestart), SELF.lastRestart, SELF.lastRestart = std::max(SELF.lastRestart, OTHER.lastRestart))
		...
		double operator[](const char* key) const {
			CLASP_CORE_STATS(CLASP_STAT_GET, (*this), )
				return -1.0;
		}
		static const char* keys(const char* path) {
			if (!path || !*path) return CLASP_CORE_STATS(CLASP_STAT_KEY, , );
			return 0;
		}
		...
		CLASP_CORE_STATS(CLASP_STAT_DEFINE, , )
	};
...
#define CLASP_EXTENDED_STATS(STAT, SELF, OTHER)     \
	...
	STAT(Array learnts;  /**< lemmas of type t-1    */, "lemmas"           , SELF.lemmas()   , )   \
	STAT(Array lits;     /**< lits of type t-1      */, "learntLits"       , SELF.learntLits(), )   \
	...
	STAT(             , "conflictLemmas", SELF.lemmas(Constraint_t::learnt_conflict), SELF.learnts[0] += OTHER.learnts[0]) \
	STAT(             , "loopLemmas"    , SELF.lemmas(Constraint_t::learnt_loop)    , SELF.learnts[1] += OTHER.learnts[1]) \
	STAT(             , "otherLemmas"   , SELF.lemmas(Constraint_t::learnt_other)   , SELF.learnts[2] += OTHER.learnts[2]) \
	STAT(             , "conflictLits"  , SELF.lits[Constraint_t::learnt_conflict-1], SELF.lits[0]    += OTHER.lits[0])    \
	STAT(             , "loopLits"      , SELF.lits[Constraint_t::learnt_loop-1]    , SELF.lits[1]    += OTHER.lits[1])    \
	STAT(             , "otherLits"     , SELF.lits[Constraint_t::learnt_other-1]   , SELF.lits[2]    += OTHER.lits[2])
		...
		double operator[](const char* key) const {
			CLASP_EXTENDED_STATS(CLASP_STAT_GET, (*this), )
				return -1.0;
		}
		static const char* keys(const char* path) {
			if (!path || !*path) { return CLASP_EXTENDED_STATS(CLASP_STAT_KEY, , ); }
			return 0;
		}
		...
		CLASP_EXTENDED_STATS(CLASP_STAT_DEFINE, , )
	};
...
#define CLASP_JUMP_STATS(STAT, SELF, OTHER)     \
	...
	STAT(uint64 bounded;  , STRING(bounded)  , SELF.bounded  , SELF.bounded  += SELF.bounded)  \
	STAT(uint64 jumpSum;  , STRING(jumpSum)  , SELF.jumpSum  , SELF.jumpSum  += SELF.jumpSum)  \
	STAT(uint64 boundSum; , STRING(boundSum) , SELF.boundSum , SELF.boundSum += SELF.boundSum) \
...
#undef MAX_MEM
		...
		double operator[](const char* key) const {
			CLASP_JUMP_STATS(CLASP_STAT_GET, (*this), )
				return -1.0;
		}
		static const char* keys(const char* path) {
			if (!path || !*path) { return CLASP_JUMP_STATS(CLASP_STAT_KEY, , ); }
			return 0;
		}
		...
		CLASP_JUMP_STATS(CLASP_STAT_DEFINE, , )
	};
	...
	struct SolverStats : public CoreStats {
		...
		const char* keys(const char* path) const {
			if (!path || !*path) {
				if (jumps && extra) { return "jumps.\0extra.\0" CLASP_CORE_STATS(CLASP_STAT_KEY, , ); }
				if (extra) { return "extra.\0" CLASP_CORE_STATS(CLASP_STAT_KEY, , ); }
				if (jumps) { return "jumps.\0" CLASP_CORE_STATS(CLASP_STAT_KEY, , ); }
...
#undef CLASP_JUMP_STATS
	...
	struct ClauseRep {
		...
		uint32     size; /*!< Size of array of literals. */
		ClauseInfo info; /*!< Additional clause info.    */
		bool isImp() const { return size > 1 && size < 4; }
	};
	//! Primitive representation of an implicit short clause.
	struct ShortRep : ClauseRep {
		ShortRep(const Literal* c, uint32 n, const ClauseInfo& i = ClauseInfo()) {
			assert(3 >= n);  std::memcpy(buf, c, n*sizeof(Literal));
			lits = buf; size = n; info = i;
		}
		ShortRep(Literal x, Literal y = negLit(0), Literal z = negLit(0)) {
			lits = buf; size = 0;
			if (x != negLit(0)) { buf[size++] = x; }
			if (y != negLit(0)) { buf[size++] = y; }
			if (z != negLit(0)) { buf[size++] = z; }
		}
		ShortRep& type(ConstraintType t) { info.setType(t); return *this; }
		Literal buf[3];
	};
