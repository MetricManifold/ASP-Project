...
#define CLASP_ASP_OPTIONS(OPTION, SELF) \
OPTION("supp-models"  , supp_models, X(flag())    , "Compute supported models (no unfounded set check)", SET_LEQ(SELF.suppMod, 1u)) \
OPTION("eq,@1"        , eq         , X(arg("<n>")), "Configure equivalence preprocessing\n" \
	"      Run for at most %A iterations (-1=run to fixpoint)", SET_OR_FILL(SELF.iters)) \
OPTION("backprop!,@1" , backprop   , X(flag())    , "Use backpropagation in ASP-preprocessing", SET_LEQ(SELF.backprop,1u)) \
OPTION("no-gamma,@1"  , no_gamma   , X(flag())    , "Do not add gamma rules for non-hcf disjunctions", SET_LEQ(SELF.noGamma,1u)) \
OPTION("eq-dfs,@2"    , eq_dfs     , X(flag())    , "Enable df-order in eq-preprocessing", SET_LEQ(SELF.dfOrder,1u)) \
OPTION("trans-ext!,@1", trans_ext  , _0           , "Configure handling of Lparse-like extended rules\n" \
	...
	_2("no"    , Asp::LogicProgram::mode_native)          , _2("all" , Asp::LogicProgram::mode_transform),    \
	_2("choice", Asp::LogicProgram::mode_transform_choice), _2("card", Asp::LogicProgram::mode_transform_card),\
	_2("weight", Asp::LogicProgram::mode_transform_weight), _2("scc" , Asp::LogicProgram::mode_transform_scc),\
	_2("integ" , Asp::LogicProgram::mode_transform_integ) , _2("dynamic", Asp::LogicProgram::mode_transform_dynamic)))
