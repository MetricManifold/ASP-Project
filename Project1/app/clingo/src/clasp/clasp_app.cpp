...
#include "../../../../libclasp/clasp/dependency_graph.h"
...
namespace Clasp {
	...
	namespace Cli {
		...
		ClaspAppOptions::ClaspAppOptions() : outf(0), onlyPre(false), printPort(false), ifs(' '), outLbd(Activity::MAX_LBD), inLbd(Activity::MAX_LBD) {
		...
		void ClaspAppOptions::initOptions(ProgramOptions::OptionContext& root) {
			...
			basic.addOptions()
				...
				("print-portfolio,g", flag(printPort), "Print default portfolio and exit")
		...
		void ClaspAppBase::initOptions(ProgramOptions::OptionContext& root) {
			...
			ProgramOptions::OptionContext::option_iterator it = root.find("verbose");
			it->get()->value()->defaultsTo("1");
		}

		void ClaspAppBase::validateOptions(const ProgramOptions::OptionContext&, const ProgramOptions::ParsedOptions& parsed) {
			if (!claspAppOpts_.validateOptions(parsed) || !claspConfig_.finalize(parsed)) {
		...
		void ClaspAppBase::printTemplate() const {
			...
			printf("# A portfolio file contains a (possibly empty) list of configurations.\n"
				...
				"#\n");
			for (const char* p = ClaspCliConfig::getPortfolio(ClaspCliConfig::portfolio_def); *p;) {
				printf("%s\n", p);
				p += strlen(p) + 1;
			}
		}
		...
		void ClaspAppBase::printDefaultConfigs() const {
			...
			std::string cmd;
			for (int i = ClaspCliConfig::config_default + 1; i != ClaspCliConfig::config_max_value + 1; ++i) {
				ClaspCliConfig::NamedConfig nc = ClaspCliConfig::getDefaults(static_cast<ClaspCliConfig::DefaultConfig> (i));
				printf("[%s]:\n%*c", nc.name, minW - 1, ' ');
				cmd = nc.opts;
		...
		bool ClaspAppBase::updateConfig(const std::string& solvingOptions, bool resetOptions) {
			...
			ProgramOptions::OptionContext opts;
			claspConfig_.addOptions(opts);
			ProgramOptions::ParsedOptions parsed;
			parsed.assign(ProgramOptions::parseCommandString(solvingOptions, opts));
			if (resetOptions) {
				opts.assignDefaults(parsed);
			}
			return claspConfig_.finalize(parsed);
		}
		...
		Output* ClaspAppBase::createOutput(ProblemType f) {
			...
			if (claspAppOpts_.outf != ClaspAppOptions::out_json || claspAppOpts_.onlyPre) {
				...
				out.reset(new TextOutput(verbose(), outFormat, claspAppOpts_.ifs));
			}
			else {
		...
		void ClaspApp::run(ClaspFacade& clasp) {
			...
			while (prg.parseProgram(input) && handlePostGroundOptions(prg)) {
				...
				if (!inc || !input.skipWhite() || *input != '9' || !clasp.update().ok()) {
					break;
				}
